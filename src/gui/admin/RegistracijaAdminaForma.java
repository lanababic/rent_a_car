...
public class RegistracijaAdminaForma extends JDialog {
	...
	public RegistracijaAdminaForma(OsobaMenadzer osobaMenadzer) {
		setTitle("Registracija novog Admina");
		setModal(true);
		setBounds(100, 100, 400, 500);
		getContentPane().setLayout(null);
		setLocationRelativeTo(null);

		JLabel lblNaslov = new JLabel("Unos podataka za Administratora");
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (ime.isEmpty() || prezime.isEmpty() || korIme.isEmpty() || lozinka.isEmpty()) {
						JOptionPane.showMessageDialog(RegistracijaAdminaForma.this, "Sva polja moraju biti popunjena!", "Greška", JOptionPane.WARNING_MESSAGE);
						return;
					}

					if (osobaMenadzer.korisnickoImePostoji(korIme)) {
						JOptionPane.showMessageDialog(RegistracijaAdminaForma.this, "Korisničko ime već postoji!", "Greška", JOptionPane.ERROR_MESSAGE);
						return;
					}

					osobaMenadzer.registrujAdmina(ime, prezime, pol, datumRodj, telefon, adresa, korIme, lozinka, sprema, staz);
					JOptionPane.showMessageDialog(RegistracijaAdminaForma.this, "Administrator uspešno registrovan!");
					dispose();

				} catch (DateTimeParseException ex) {
					JOptionPane.showMessageDialog(RegistracijaAdminaForma.this, "Format datuma mora biti YYYY-MM-DD!", "Greška", JOptionPane.ERROR_MESSAGE);
				} catch (NumberFormatException ex) {
					JOptionPane.showMessageDialog(RegistracijaAdminaForma.this, "Staž mora biti broj!", "Greška", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
